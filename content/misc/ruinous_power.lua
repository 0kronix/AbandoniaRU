...
G.C.RUINOUS_POWER = HEX("b26370")
G.C.RUINOUS_POWER_SECONDARY = HEX("272727")

SMODS.ConsumableType {
    key = "ruinous_power",
    collection_rows = { 5, 5 },
    shop_rate = 0.0,
    primary_colour = G.C.RUINOUS_POWER,
    secondary_colour = G.C.RUINOUS_POWER_SECONDARY,
}

SMODS.current_mod.custom_card_areas = function(game)
    game.ruinous_powers = CardArea(
        game.consumeables.T.x, 
        game.consumeables.T.y + game.consumeables.T.h + 0.35,
        game.consumeables.T.w, 
        game.consumeables.T.h,
        { type = 'joker', card_limit = 1, highlight_limit = 1 }
    )
    game.ruinous_powers.config.align_buttons = true
    
   G.ruinous_powers = game.ruinous_powers
end
